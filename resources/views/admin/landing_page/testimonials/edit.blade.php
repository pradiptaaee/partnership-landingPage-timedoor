...
<div class="w-full p-6 bg-gray-50 h-[calc(100vh-80px)] overflow-hidden flex flex-col">
    ...
    <div class="flex justify-between items-center mb-6 shrink-0">
        <div>
            ...
            <p class="text-gray-500 text-sm">Perbarui data review orang tua murid</p>
        </div>
            ...
            <div class="flex flex-col lg:flex-row h-full">
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="student_name" value="{{ old('student_name', $testimonial->student_name) }}" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 outline-none transition">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Level / Kursus</label>
                                <input type="text" name="course_name" value="{{ old('course_name', $testimonial->course_name) }}" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 outline-none transition">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="review" rows="5" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 outline-none transition resize-none" required>{{ old('review', $testimonial->review) }}</textarea>
                        </div>

                        {{-- Info Mode Edit --}}
                        <div class="p-4 bg-yellow-50 rounded-lg border border-yellow-100 flex items-start gap-3">
                            <i class="bi bi-info-circle-fill text-yellow-600 mt-0.5"></i>
                            <p class="text-xs text-yellow-700 leading-relaxed">
                                <strong>Mode Edit:</strong> Kosongkan bagian upload foto di sebelah kanan jika Anda tidak ingin mengganti foto profil yang sekarang.
                            </p>
                        </div>
                    </div>
                ...
                <div class="w-full lg:w-1/2 bg-gray-900 relative group h-full">
                    <label for="dropzone-file" class="flex flex-col items-center justify-center w-full h-full cursor-pointer relative overflow-hidden">
                        ...
                        {{-- Prompt (Hidden karena defaultnya menampilkan foto lama) --}}
                        ...
                        {{-- Preview Image (Menampilkan Foto Lama dari Database) --}}
                        <div id="preview-container" class="absolute inset-0 w-full h-full bg-black">
                            <img id="preview-image" 
                                 src="{{ Storage::url($testimonial->parent_image) }}" 
                                 class="w-full h-full object-cover opacity-100 group-hover:opacity-70 transition duration-300">
