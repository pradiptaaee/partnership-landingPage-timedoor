...
<div class="flex-1 p-8 bg-white min-h-screen font-sans flex flex-col">
    ...
    <div class="flex-grow bg-white rounded-2xl border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] overflow-hidden flex flex-col">
        <form action="{{ route('admin.projects.update', $project->id) }}" method="POST" enctype="multipart/form-data" class="flex flex-col lg:flex-row h-full">
            ...
            <div class="w-full lg:w-5/12 p-8 border-b lg:border-b-0 lg:border-r border-gray-100 flex flex-col h-full bg-white">
                <div class="space-y-6 flex-grow overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    <div>
                        ...
                        <input type="text" name="student_name" value="{{ old('student_name', $project->student_name) }}"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" name="age" value="{{ old('age', $project->age) }}"
                                   class="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-gray-800 focus:bg-white focus:border-[#0f5132] outline-none transition text-sm font-medium" required>
                    ...
                    <div>
                        ...
                        <input type="text" 
                            ...
                            value="{{ old('project_type.id', $project->project_type['id'] ?? '') }}"
                               class="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-gray-800 focus:bg-white focus:border-[#0f5132] outline-none transition text-sm font-medium" required>
                    ...
                    <div class="p-4 bg-emerald-50 rounded-xl border border-emerald-100 flex gap-3 items-start text-[#0f5132]">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-emerald-700 leading-relaxed">Sistem akan otomatis menerjemahkan ulang ke Inggris & Jepang jika judul Indonesia diubah.</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="w-full lg:w-7/12 bg-gray-900 relative group overflow-hidden flex items-center justify-center min-h-[300px]">
                ...
                <div class="w-full h-full relative">
                    <img id="preview-image" src="{{ Storage::url($project->project_image) }}" class="w-full h-full object-contain transition-all duration-500 group-hover:scale-95 group-hover:opacity-60">
                ...
                <div class="absolute bottom-6 left-6 right-6 z-10 pointer-events-none">
                    <p class="text-[10px] text-[#4ade80] font-bold uppercase tracking-wider mb-1 flex items-center gap-1">
                        <span class="w-1.5 h-1.5 rounded-full bg-[#4ade80] animate-pulse"></span> Gambar Saat Ini
                    </p>
                    <p id="file-name" class="text-white font-medium text-sm truncate opacity-90">{{ basename($project->project_image) }}</p>
                </div>
            </div>
        </form>
    </div>
