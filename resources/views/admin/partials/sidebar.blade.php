<aside id="sidebar-admin" class="fixed inset-y-0 left-0 w-72 bg-[#050505] border-r border-white/5 flex flex-col z-50 font-sans shadow-2xl transition-all duration-300">

    {{-- 1. HEADER: Logo SVG --}}
    <div class="h-28 flex items-center justify-center relative overflow-hidden shrink-0 border-b border-white/5">
        {{-- Ambient Glow Background (Efek cahaya di belakang logo) --}}
        <div class="absolute top-0 w-full h-full bg-gradient-to-b from-emerald-900/20 to-transparent blur-3xl pointer-events-none"></div>

        {{-- Container Logo --}}
        <div class="relative z-10 w-48 px-4 opacity-90 hover:opacity-100 transition-opacity duration-300">
            {{-- SVG LOGO START --}}
            <svg viewBox="0 0 166 43" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-full h-auto drop-shadow-lg">
    ...
    {{-- 2. NAVIGATION --}}
    <nav class="flex-1 overflow-y-auto px-4 py-4 space-y-2 no-scrollbar">

        {{-- SECTION: MAIN --}}
        <div class="px-4 mt-2 mb-2">
            <span class="text-[10px] uppercase tracking-[0.15em] text-gray-600 font-bold">Main Menu</span>
        </div>

        {{-- Dashboard --}}
        <a href="#"
           class="group flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-300
                  text-gray-400 hover:text-white hover:bg-white/5 border border-transparent hover:border-white/5">
            <i class="bi bi-grid-1x2-fill text-lg group-hover:text-emerald-400 transition-colors"></i>
            <span class="text-sm font-medium tracking-wide">Dashboard</span>
        </a>

        {{-- SECTION: PARTNERSHIP --}}
        <div class="px-4 mt-8 mb-2">
            <span class="text-[10px] uppercase tracking-[0.15em] text-gray-600 font-bold">Partnership</span>
        </div>

        {{-- Partners --}}
        <a href="{{ route('admin.partners.index') }}"
           class="group flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-300 border border-transparent
                  {{ request()->routeIs('admin.partners.*') 
                     ? 'bg-gradient-to-r from-emerald-500/10 to-transparent text-emerald-400 border-emerald-500/20 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.05)]' 
                     : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
            <i class="bi bi-briefcase-fill text-lg {{ request()->routeIs('admin.partners.*') ? 'text-emerald-400' : 'group-hover:text-emerald-400 transition-colors' }}"></i>
            <span class="text-sm font-medium tracking-wide">Partners</span>
        </a>
    ...
    {{-- 3. FOOTER: Glassmorphism Profile --}}
    <div class="p-4 mx-4 mb-4 shrink-0">
        <div class="bg-gradient-to-r from-gray-900 to-gray-800 rounded-2xl p-4 border border-white/5 shadow-2xl relative overflow-hidden group">
            
            {{-- Background Glow --}}
            <div class="absolute -right-6 -top-6 w-20 h-20 bg-emerald-500/20 blur-[40px] group-hover:bg-emerald-500/30 transition-all duration-500"></div>

            <div class="flex items-center justify-between relative z-10">
                <div class="flex items-center gap-3">
                    <img src="https://ui-avatars.com/api/?name=Admin&background=10B981&color=fff" class="w-9 h-9 rounded-full ring-2 ring-emerald-500/30 shadow-sm" alt="Admin">
                    <div class="flex flex-col">
                        <span class="text-sm font-bold text-white tracking-wide">Admin</span>
                        <span class="text-[10px] text-emerald-400 uppercase font-semibold">Online</span>
                    </div>
                </div>
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-8 h-8 flex items-center justify-center rounded-lg bg-white/5 hover:bg-red-500/20 hover:text-red-400 text-gray-400 transition-all duration-300" title="Logout">
