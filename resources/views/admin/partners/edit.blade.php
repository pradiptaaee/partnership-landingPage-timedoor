...
<div class="flex-1 p-8 bg-white min-h-screen font-sans flex flex-col">
    ...
    <div class="flex-grow bg-white rounded-2xl border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] overflow-hidden flex flex-col">
        ...
        <form action="{{ route('admin.partners.update', $partner->id) }}" method="POST" enctype="multipart/form-data" class="flex flex-col lg:flex-row h-full">
            @csrf
            @method('PUT')
            ...
            <div class="w-full lg:w-7/12 p-8 border-b lg:border-b-0 lg:border-r border-gray-100 flex flex-col h-full bg-white">
                ...
                <div class="space-y-6 flex-grow overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    {{-- INFO BOX: KEGIATAN --}}
                    <div class="p-4 bg-emerald-50 rounded-xl border border-emerald-100 flex items-center justify-between gap-4">
                        ...
                        <a href="{{ route('admin.activity.index', ['partner_id' => $partner->id]) }}" class="text-xs font-bold text-[#0f5132] hover:underline whitespace-nowrap">
                            Lihat Kegiatan &rarr;
                        </a>
                    </div>
            ...
            <div class="w-full lg:w-5/12 bg-gray-50 flex flex-col min-h-[400px] lg:min-h-auto border-l border-gray-100">
                ...
                <div class="relative group flex-grow flex flex-col items-center justify-center p-8 overflow-hidden">
                    ...
                    {{-- Hidden Input --}}
                    <input id="dropzone-file" name="logo" type="file" class="hidden" accept="image/*" onchange="previewImage(event)" />
                    
                    {{-- Trigger Label (Overlay) --}}
                    <label for="dropzone-file" class="absolute inset-0 z-20 cursor-pointer flex flex-col items-center justify-center bg-white/80 opacity-0 group-hover:opacity-100 transition-all duration-300 backdrop-blur-sm">
                        <div class="w-16 h-16 mb-3 rounded-full bg-white shadow-md border border-gray-100 flex items-center justify-center text-[#0f5132] transform translate-y-4 group-hover:translate-y-0 transition duration-300">
                    ...
                    <div class="absolute bottom-6 left-0 right-0 text-center pointer-events-none">
                        <p class="text-[10px] text-[#0f5132] font-bold uppercase tracking-wider inline-flex items-center gap-1 bg-white/50 px-2 py-1 rounded-full backdrop-blur-sm border border-gray-200">
                ...
                <div class="p-6 bg-white border-t border-gray-100">
                    ...
                    <button type="button" onclick="confirmDelete()" class="w-full px-4 py-2.5 rounded-lg border border-red-100 bg-red-50 text-red-600 text-sm font-bold hover:bg-red-100 hover:text-red-700 transition flex items-center justify-center gap-2">
