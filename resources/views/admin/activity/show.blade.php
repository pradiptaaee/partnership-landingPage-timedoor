    ...
    <div class="container py-4">
        ...
        <div class="mb-5">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb flex items-center mb-2 gap-2 text-sm font-medium">
                    <li class="breadcrumb-item"><a href="{{ route('admin.activity.index') }}"
                            class="text-decoration-none text-[#0f5132]">Activities </a></li>
                    <li> / </li>
            ...
            <div class="w-full flex justify-between items-end">
                <div>
                    ...
                    <div class="flex gap-4 mt-2">
                        <span
                            class="text-xs font-bold text-[#0f5132] uppercase tracking-wider bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-100">
                            <i class="bi bi-tag-fill me-1"></i> {{ $activity->category_activity }}
                        </span>
                        <span
                            class="text-xs font-bold text-gray-500 uppercase tracking-wider bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                ...
                <a href="{{ route('admin.activity.index') }}"
                    class="px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-gray-600 text-sm font-bold shadow-sm hover:bg-gray-50 transition flex items-center gap-2">
        ...
        <div
            class="flex-grow bg-white rounded-2xl border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] overflow-hidden">
            <div class="flex flex-col lg:flex-row h-full">
                ...
                <div
                    ...
                    <div class="space-y-8 flex-grow custom-scrollbar">
                        ...
                        {{-- Detail Partner --}}
                        <div>
                            <label class="block text-xs font-bold text-[#0f5132] uppercase tracking-wider mb-3">Partner
                                Pelaksana</label>
                            <div class="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                                @if ($activity->partner && $activity->partner->logo)
                                    <img src="{{ asset('storage/' . $activity->partner->logo) }}"
                                        class="w-12 h-12 rounded-lg object-contain bg-white p-1 border border-gray-200">
                                @else
                                    <div
                                        class="w-12 h-12 bg-white rounded-lg flex items-center justify-center border border-gray-200 text-gray-400">
                                        <i class="bi bi-building"></i>
                                    </div>
                                @endif
                                <div>
                                    <h4 class="text-sm font-bold text-gray-800 mb-0">
                                        {{ $activity->partner->name ?? 'Internal / Partner Tidak Ditemukan' }}</h4>
                                    <p class="text-[10px] text-gray-500 uppercase tracking-tighter">
                                        {{ $activity->partner->category ?? '-' }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        {{-- Deskripsi --}}
                        <div>
                            <label class="block text-xs font-bold text-[#0f5132] uppercase tracking-wider mb-3">Deskripsi
                                Lengkap</label>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                @if ($activity->location)
                                    <div class="p-4 bg-gray-50 rounded-xl">
                                        <label
                                            class="block text-[10px] font-bold text-gray-400 uppercase mb-1">Lokasi</label>
                                        <span class="text-sm font-semibold text-gray-800"><i
                                                class="bi bi-geo-alt me-2 text-[#0f5132]"></i>{{ $activity->location }}</span>
                                    </div>
                                @endif
                                @if ($activity->speaker)
                                    <div class="p-4 bg-gray-50 rounded-xl">
                                        <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1">Pembicara /
                                            Mentor</label>
                                        <span class="text-sm font-semibold text-gray-800"><i
                                                class="bi bi-person me-2 text-[#0f5132]"></i>{{ $activity->speaker }}</span>
                                    </div>
                                @endif
                            </div>
                        @endif
                ...
                <div class="w-full lg:w-5/12 bg-gray-50 overflow-y-auto custom-scrollbar flex flex-col p-8">
                    ...
                    {{-- Featured Image --}}
                    <div class="mb-8">
                        <label class="block text-xs font-bold text-[#0f5132] uppercase tracking-wider mb-4">Gambar
                            Utama</label>
                        <div
                            class="relative w-full aspect-video bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
                    ...
                    {{-- Gallery flex --}}
                    <div>
                        <label class="block text-xs font-bold text-[#0f5132] uppercase tracking-wider mb-4">Galeri
                            Dokumentasi</label>
                        @if ($activity->photos && count($activity->photos) > 0)
                            <div class="flex flex-wrap gap-2">
                                @foreach ($activity->photos as $photo)
                                    <div
                                        class="relative group w-20 h-20 bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm flex-shrink-0">
                            ...
                            <p class="mt-3 text-[10px] text-gray-400 italic font-medium">* Klik gambar untuk memperbesar
                                </p>
                        @else
                            <div class="p-6 bg-white rounded-2xl border border-dashed border-gray-200 text-center">
                                ...
                                <p class="text-[10px] text-gray-400 uppercase font-bold tracking-widest">Belum ada foto
                                    galeri</p>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="mt-auto pt-8">
                        <div class="p-4 bg-white rounded-xl border border-gray-100 shadow-sm">
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <span
                                    class="text-xs font-mono text-gray-500">ACT-{{ str_pad($activity->id, 5, '0', STR_PAD_LEFT) }}</span>
                            </div>
                        </div>
                    </div>
